...
describe('API generate --> /api/generate/', function() {
  ...
  describe('GET', function() {
    it(`GIVEN ${urlAPI} with parameters ext=asciidoc and template=basic WHEN send GET request THEN should return the readme write in asciicode for a basic project`, function(done) {
      superagent.get(`${host}${urlAPI}?ext=asciidoc&template=basic`).end(function(e, res) {
        fs.readFile('src/templates/asciidoc/basic.asciidoc', function(err, data) {
          let resultat = JSON.parse(res.text);
          expect(res.status).to.eql(200);
